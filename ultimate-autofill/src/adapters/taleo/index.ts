...
export const taleoAdapter: ATSAdapter = {
    ...
    detect(doc: Document): ATSDetectionResult {
        ...
        if (/oraclecloud\.com|taleo\.net/i.test(url)) { conf += 0.6; signals.push('url'); }
        if (doc.querySelector('#OracleFusionApp,oracle-apply-flow')) { conf += 0.3; signals.push('oracle-app'); }
