...
(() => {
  const MENU_ID = "optimhireProfileBackup";
  const PAGE = "tabs/profileBackup.html";
  const ensureMenu = () => {
    try {
  ...
  try { chrome.runtime.onInstalled.addListener(ensureMenu); } catch (e) {}
  try { chrome.runtime.onStartup.addListener(ensureMenu); } catch (e) {}
  try {
  ...
  try {
    chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
      if (msg && msg.type === "OPEN_PROFILE_BACKUP") {
        try { chrome.tabs.create({ url: chrome.runtime.getURL(PAGE) }); } catch (e) {}
        sendResponse({ ok: true });
        return true;
      }
